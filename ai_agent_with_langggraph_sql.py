# -*- coding: utf-8 -*-
"""AI_Agent_with_langggraph_SQL.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1Ep1ibB3Aa_GQEdRVS1BNzPbTTntCwJXx

<a href="https://colab.research.google.com/github/KevinCY-Kim/Deeplearning/blob/main/AI_Agent_with_langggraph_SQL.ipynb" target="_parent"><img src="https://colab.research.google.com/assets/colab-badge.svg" alt="Open In Colab"/></a>
"""
...
def rag_agent(state: AgentState):  # RAG 검색 기반 질병 설명 생성
    ...
    retriever = vectorstore.as_retriever() # retriever 정의
...
def sql_agent(state: AgentState):  # DB에서 질병 대처방안 조회
    ...
    return {**state, "disease_info": state["disease_info"] + "\n" + "\n".join(results)}
                                        # 질병 정보와       SQL 결과(care)를 문자열로 합친거임
                                        # 예시) 감기 : 뜨신물을 마시고, 충분한 숙면을 취하시오.
...
# 바이브 코딩을 이용한 multi Agent 실습
# text 파일 내용
# 라면은 짠맛이 강하고, 숙취해소에 좋다.
# 돈까스는 바삭바삭하며, 저체중인 사람들에게 좋다
# 샐러드는 깔끔한 맛이 나며, 비만인 사람들에게 좋다.
# SQL 내용
# name  price
# 라면   10000원
# 돈까스  20000원
# 샐러드 15000원
# 쿼리 : "나는 현재 비만인데, 어떤 음식을 먹으면 좋고 얼마가 나올지 알려줘"
...
# 0. RAG 벡터스토어 생성 (최초 1회 실행 후 주석처리 가능)
def build_rag_chroma():
    with open("rag_data/menu.txt", "w", encoding="utf-8") as f:  # 텍스트 파일 저장
        f.write("""라면은 짠맛이 강하고, 숙취해소에 좋다\n
                    돈까스는 바삭바삭하며, 저체중인 사람들에게 좋다.\n
                    샐러드는 깔끔한 맛이 나며, 비만인 사람들에게 좋다.""")

    loader = TextLoader("rag_data/menu.txt", encoding="utf-8")  # 텍스트 로드
    docs = loader.load()  # 문서 리스트 로드
    splitter = CharacterTextSplitter(chunk_size=300, chunk_overlap=20)  # 텍스트 분할기
    split_docs = splitter.split_documents(docs)  # 문서 나누기
...
cursor.executemany("INSERT INTO menu (name, price) VALUES (?, ?)", data)
conn.commit()
conn.close()
...
# 4. 에이전트 정의
extractor_prompt = PromptTemplate.from_template("""
사용자의 질문에서 메뉴에 해당하는 단어 또는 구를 추출하세요.
결과는 쉼표로 구분된 문자열로 출력하세요.
질문: {query}
""")  # 메뉴 추출 프롬프트

def extractor_agent(state: AgentState):  # 메뉴 추출 함수
    chain = extractor_prompt | llm  # 프롬프트 체인
    symptoms = chain.invoke({"query": state["query"]})  # LLM 실행
    return {**state, "menu": symptoms.strip()}  # 상태에 추가

matcher_prompt = PromptTemplate.from_template("""
다음 메뉴 목록을 바탕으로 가장 가능성 높은 음식 이름 3개를 쉼표로 추정하세요.
메뉴: {menu}
""")  # 메뉴 후보 추정 프롬프트

def matcher_agent(state: AgentState):  # 질병 후보 추정
    chain = matcher_prompt | llm
    candidates = chain.invoke({"menu": state["menu"]})
    return {**state, "menu_candidates": candidates.strip()}
...
def rag_agent(state: AgentState):  # RAG 검색 기반 질병 설명 생성
    ...
    rag_prompt = PromptTemplate.from_template("""
                                                ...
                                                다음 참고 문서를 바탕으로 관련 음식 정보와 음식 가격을 요약해 주세요:
                                                {context}
                                                """)
    rag_chain = create_stuff_documents_chain(llm, rag_prompt)
    docs = retriever.get_relevant_documents(state["menu"])
    # print(docs)로 실제 물렸는지확인

    rag_response = rag_chain.invoke({"menu": state["menu"], "context": docs})
    return {**state, "menu_info": rag_response.strip()}


def sql_agent(state: AgentState):
    conn = sqlite3.connect("menu.db")
    cursor = conn.cursor()
    names = [n.strip() for n in state["menu_candidates"].split(",")]
    results = []
    for name in names:
        cursor.execute("SELECT price FROM menu WHERE name = ?", (name,))
        row = cursor.fetchone()
        if row:
            results.append(f"{name} 가격: {row[0]}원")
        else:
            results.append(f"{name}: 가격 정보 없음")
    conn.close()
    return {**state, "cost": "\n".join(results)}  # ✅ 가격 정보는 cost로

answer_prompt = PromptTemplate.from_template("""
추천 메뉴은 다음과 같습니다: {menu}
...
# 6. 실행 예시
query = "나는 현재 비만인데, 어떤 음식을 먹으면 좋고 얼마가 나올지 알려줘"  # 사용자 질문
result = app.invoke({"query": query})  # 실행
